...
# 40km wea
dens_40km = narw_40km |>
  dplyr::filter(!is.na(wea_40km)) |>
  sf::st_drop_geometry() |>
  dplyr::group_by(month, wea_40km) |> 
  dplyr::summarise(density = quantile(value, probs = 0.9, na.rm = TRUE)) |>
  dplyr::ungroup()
...
```{r trigger, fig.width = 8, fig.height = 75, fig.align = 'left'}
...
for (i in wea_order) {
    ...
    # filter data by wea bufferd by 40km
    wea_40km_filt = dens_40km |>
      dplyr::filter(wea_40km == i) |>
      dplyr::mutate(constraint = '40km') |>
      dplyr::rename(wea = wea_40km)
    ...
    # combine
    data_gg = dplyr::bind_rows(wea_filt, wea_10km_filt, wea_40km_filt, region_filt, coast_filt) |>
      dplyr::mutate(constraint = factor(constraint, levels = c('0km', '10km', '40km', wea_region, 'Coastwide'))) |>
