...
wea_order = c('OCS-A 0562', 'OCS-A 0564', 'OCS-A 0567', 'OCS-A 0568', 'OCS-A 0486', 'OCS-A 0517', 'OCS-A 0487', 'OCS-A 0500', 'OCS-A 0501', 'OCS-A 0534', 'OCS-A 0520', 'OCS-A 0521', 'OCS-A 0522', 'OCS-A 0512', 'OCS-A 0544', 'OCS-A 0537', 'OCS-A 0538', 'OCS-A 0539', 'OCS-A 0541', 'OCS-A 0542', 'OCS-A 0549', 'OCS-A 0499', 'OCS-A 0498', 'OCS-A 0532', 'OCS-A 0482', 'OCS-A 0519', 'OCS-A 0490', 'OCS-A 0483', 'OCS-A 0508', 'OCS-A 0545', 'OCS-A 0546')
```
...
# narw habitat selection
narw_R = here::here('data', 'narw_dens_R.csv') |>
  readr::read_csv() |>
  dplyr::mutate(month = ordered(month, levels = c(month.abb[c(10:12, 1:9)])),
                lease_number = factor(lease_number, levels = wea_order))
...
plt_list = purrr::map(.x = wea_order, .f = function(.x) {
  ...
  # format data (coastwide instead of regional)
  dens_data = trigger_data_coast |> 
    dplyr::filter(wea == .x)
  ...
  plt1 = ggplot() + 
    ...
    theme(strip.background = element_blank(), 
          ...
          plot.margin = margin(t = 0, b = 7, r = 7, l = 7)) + 
    labs(x = NULL, y = NULL) + 
    guides(color = 'none') + 
  ...
  plt2 = ggplot(data = box_data, aes(x = month, y = value, fill = month)) +
    ...
    theme_cowplot() + 
    labs(x = NULL, y = 'density', title = company) + 
    guides(fill = 'none') + 
    scale_x_discrete(labels = function(x) substr(x, 1, 3)) + 
    theme(axis.text.x = element_blank())
    # + geom_hline(yintercept = 1, linetype = 'dashed') # floor((4 / 100) / 3.429904 * 100)
  
  # get y-axis precision for boxplot
  plt2_build = ggplot2::ggplot_build(plt2)
  plt2_ylabs = plt2_build$layout$panel_params[[1]]$y$get_labels()
  plt2_prec = stringr::str_split_i(string = na.omit(plt2_ylabs), pattern = '\\.', i = 2) |> nchar() |> unique()
  plt3_prec = paste0('0.', paste(rep('0', plt2_prec - 1), collapse = ''), '1') |>
    as.numeric()
  
  # anomoly plot
  plt3 = ggplot(data = dens_R, aes(x = month, y = log_anomaly, fill = behavior)) + 
    geom_col(color = 'black', alpha = 0.85, width = 0.7) + 
    scale_y_continuous(breaks = 0, labels = scales::label_number(accuracy = plt3_prec)) +
    geom_hline(yintercept = 0, color = 'black') + 
    scale_fill_manual(values = c('Selection (R > 1)' = '#d73027', 'Avoidance (R < 1)' = '#4575b4')) + 
    labs(x = NULL, y = 'ln(R)') + 
    theme_cowplot() + 
    theme(
      ...
      plot.margin = margin(t = 0, b = 0, r = 7, l = 7)
    ) 
  ...
  # combine plots
 plot_grid(plt2, plt3, plt1, ncol = 1, rel_heights = c(4/10, 2/10, 4/10), align = 'v', axis = 'left')
