...
narw_gg = narw_weas |>
  ...
  sf::st_drop_geometry() |>
  dplyr::mutate(fill = cut(value * 100, breaks = c(10, 6.3, 4, 2.5, 1.6, 1, 0.63, 0.40, 0.25, 0.16, 0.10, 0.063, 0.040, 0.025, 0.016, 0), include.lowest = TRUE))
...
In the following plots, we'll be summarizing the monthly NARW density data by active lease Wind Energy Areas (WEAs) that are roughly ordered from north to south. A look up table that relates the lease number for active lease WEAs to their corresponding company and state is provided below.
...
Below are boxplots of the monthly NARW densities by active lease WEAs (again, roughly ordered from north to south). The boxplots were drawn such that the lower whisker represents the 10th percentile, the lower hinge represents the 25th percentile, the dot represents the mean, the line represents the median, the upper hinge represents the 75th percentile, and the upper whisker represents the 90th percentile. Underneath a boxplot are Gantt charts indicating months that should impose temporal restrictions on OWD using two thresholds (blue bars), 1) Average = is the average monthly density in the WEA greater than the average annual density in the WEA and 2) Percentile (90th), is the 90th percentile of monthly density in the WEA greater than the 90th percentile of density over the entire year in the WEA? The y-axis of the Gantt charts indicates the degree that active lease WEAs were buffered (0km = not buffered, 10km = buffered by 10km, and 40km = buffered by 40km). 
...
plt_list = purrr::map(.x = wea_order, .f = function(.x) {
  ...
  dens_data = narw_dens |> 
    dplyr::filter(wea == .x) |> 
    plyr::mutate(`Percentile (90th)` = ifelse(dens_90 > ann_90, 1, NA), 
                 Average = ifelse(dens_avg > ann_avg, 1, NA)) |> 
    dplyr::select(-c(dens_90:ann_avg)) |> 
    tidyr::pivot_longer(cols = `Percentile (90th)`:Average, names_to = 'indicator', values_to = 'value') |>
    dplyr::filter(constraint %in% c('0km', '10km', '40km')) 
  ...
  plt2 = ggplot(data = box_data, aes(x = month, y = value * 100, fill = month)) +
    ...
    # combine plots
    plot_grid(plt2, plt1, ncol = 1, rel_heights = c(2/3, 1/3), align = 'v', axis = 'left')
