...
narw_inds = (narw_dens / 100 * 25) |> 
  terra::project('+proj=utm +zone=19 +ellps=GRS80 +datum=NAD83 +units=m +no_defs')
narw_dens_utm = narw_inds / (terra::cellSize(narw_inds) / (1000 * 1000)) * 100
  
# compare cell area
# test_cetmap = cell_id(x = narw_dens[[1]]) |>
#   dplyr::mutate(area = area / (1000 * 1000))
# test_utm = cell_id(x = narw_inds[[1]]) |>
#   dplyr::mutate(area = area / (1000 * 1000))
...
ggplot(data = narw_crs, aes(x = MONTH, y = VALUE, fill = CRS, group = interaction(MONTH, WEA, CRS))) +
  ...
  geom_hline(yintercept = 1, linetype = 'dashed') # 1 whale / 100km2 -> (4 whales / 342.99 km2) * 100
```
