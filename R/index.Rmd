...
For all metrics, the blue bar indicates when the monthly metric exceeded the average annual, regional density. The y-axis of the Gantt charts indicates the degree that active lease areas were buffered (0km = not buffered, 10km = buffered by 10km, and 40km = buffered by 40km).
...
```{r reg-boxplot}
reg_boxplot = ggplot(data = narw_weas |> dplyr::filter(!is.na(region)) |> dplyr::mutate(region = factor(region, levels = c('Gulf of Maine', 'Southern New England', 'Mid-Atlantic', 'Southeast')), month = factor(month, levels = month.abb[c(7:12, 1:6)])), aes(x = month, y = value, fill = month)) + 
  stat_summary(fun.data = boxplot_stat, geom = 'boxplot') + 
  stat_summary(fun = mean, geom = 'point', size = 3) + 
  theme_cowplot() + 
...
# add_lines = narw_dens |> 
#   dplyr::distinct(region, ann_reg_90, ann_reg_75, ann_reg_50, ann_reg_avg) |>
#   tidyr::pivot_longer(cols = ann_reg_90:ann_reg_avg, names_to = 'metric', values_to = 'value')
# reg_boxplot + geom_hline(data = add_lines, aes(yintercept = value), linetype = 'dashed')
```
